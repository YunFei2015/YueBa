...
static NSString * const kMARKRangeSliderThumbImage = @"slider-default7-handle";
...
//    [self addSubview:self.rangeImageView];
    ...
    self.rangeView = [[UIView alloc] init];
...
//    self.trackImageView.frame = CGRectMake(trackX, trackY, trackWidth, trackHeight);
    self.trackView.frame = CGRectMake(trackX, (height - 2) / 2, trackWidth, 2);
    self.trackView.backgroundColor = [UIColor lightGrayColor];
...
//    self.rangeImageView.frame = CGRectMake(leftX + leftInset, trackY, rangeWidth, trackHeight);
    self.rangeView.frame = CGRectMake(leftX + leftInset, (height - 2) / 2, rangeWidth, 2);
